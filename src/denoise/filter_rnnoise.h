...
typedef struct {
    ...
    // esp_rnnoise_prefer_type_t prefer_flag; /*!< The select flag about lesser CPU usage or lower INRAM usage, refer to esp_resample.h */
    int out_rb_size;           /*!< Output ringbuffer size*/
    int task_stack;            /*!< Task stack size */
    int task_core;             /*!< Task running on core */
    int task_prio;             /*!< Task priority */
    bool stack_in_ext;         /*!< Try to allocate stack in external memory */
} rnnoise_filter_cfg_t;

#define FRAME_SIZE 1024

#define RNNOISE_FILTER_BUFFER_BYTE              (FRAME_SIZE)
#define RNNOISE_FILTER_TASK_STACK               (55 * 1024)
#define RNNOISE_FILTER_TASK_CORE                (1)
#define RNNOISE_FILTER_TASK_PRIO                (10)
#define RNNOISE_FILTER_RINGBUFFER_SIZE          (2 * 1024)

#define DEFAULT_RNNOISE_FILTER_CONFIG() {          \
        .src_rate = 44100,                          \
        .src_ch = 2,                                \
        .dest_rate = 48000,                         \
        .dest_ch = 2,                               \
        .sample_bits = 16,                          \
        .max_indata_bytes = RNNOISE_FILTER_BUFFER_BYTE, \
        .out_len_bytes = RNNOISE_FILTER_BUFFER_BYTE,    \
        .complexity = 2,                            \
        .down_ch_idx = 0,                           \
        .out_rb_size = RNNOISE_FILTER_RINGBUFFER_SIZE,  \
        .task_stack = RNNOISE_FILTER_TASK_STACK,        \
        .task_core = RNNOISE_FILTER_TASK_CORE,          \
        .task_prio = RNNOISE_FILTER_TASK_PRIO,          \
        .stack_in_ext = true,                       \
    }
 ...
 *             ESP_FAIL
 */
// esp_err_t rnnoise_filter_set_src_info(audio_element_handle_t self, int src_rate, int src_ch);
